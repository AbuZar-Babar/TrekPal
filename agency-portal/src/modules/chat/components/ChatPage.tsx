...
const ChatPage = () => {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[340px,1fr]">
                ...
                <button
                  ...
                  className={`mb-2 w-full rounded-[20px] border px-4 py-4 text-left transition ${
                    isActive
                      ...
                      : 'border-transparent bg-[var(--panel)] hover:border-[var(--border)]'
                  }`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-semibold text-[var(--text)]">{room.title}</div>
      ...
      <section className="app-card flex min-h-[78vh] flex-col overflow-hidden">
        {selectedRoom ? (
          <>
            <div className="border-b border-[var(--border)] px-6 py-5">
              <div className="flex flex-wrap items-center gap-3">
                <div>
                  <div className="text-lg font-semibold tracking-tight text-[var(--text)]">{selectedRoom.title}</div>
                  <div className="mt-1 text-sm text-[var(--text-soft)]">
                    {selectedRoom.participantCount} traveler{selectedRoom.participantCount === 1 ? '' : 's'} in this offer
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-3 overflow-y-auto px-6 py-5">
                  {messages.length === 0 ? (
                    <div className="flex h-full items-center justify-center text-center">
                      <div>
                        <div className="text-lg font-semibold tracking-tight text-[var(--text)]">Start the conversation</div>
                        <p className="mt-2 text-sm text-[var(--text-muted)]">Ask travelers or confirm trip details here.</p>
                      </div>
                    </div>
                  ) : (
                    messages.map((message) => {
                      ...
                      return (
                        <div
                          key={message.id}
                          className={`max-w-[520px] rounded-[22px] px-4 py-3 ${
                            isAgency
                              ? 'ml-auto bg-[var(--primary)] text-white'
                              : 'bg-[var(--panel-strong)] text-[var(--text)]'
                          }`}
                        >
                          <div className={`text-xs font-semibold uppercase tracking-[0.14em] ${isAgency ? 'text-white/70' : 'text-[var(--text-soft)]'}`}>
                            {isAgency ? 'Agency' : message.senderName}
                          </div>
                          <div className="mt-2 whitespace-pre-wrap text-sm leading-6">{message.content}</div>
                          <div className={`mt-3 text-xs ${isAgency ? 'text-white/70' : 'text-[var(--text-soft)]'}`}>
          ...
          <div className="flex flex-1 items-center justify-center px-6 text-center">
            <div>
              <div className="text-lg font-semibold tracking-tight text-[var(--text)]">Pick an offer chat</div>
              <p className="mt-2 text-sm text-[var(--text-muted)]">Open any joined-offer room to talk to travelers in real time.</p>
            </div>
          </div>
        )}
      </section>
